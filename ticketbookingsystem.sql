...
UPDATE Event SET event_name = 'Regionals Cup Cricket Match' WHERE event_id = 3;
UPDATE Event SET ticket_price = 1500 WHERE event_id = 3;
UPDATE Event SET ticket_price = 1850 WHERE event_id = 6;
UPDATE Event SET ticket_price = 2300 WHERE event_id = 14;
UPDATE Customer set phone_number = '677-524-3000' where customer_id = 2;
UPDATE Customer set phone_number = '783-565-5000' where customer_id = 15;
UPDATE Customer set phone_number = '543-233-3000' where customer_id = 8;
...
select event_name,event_date from Event where event_date between '2024-01-01' and '2024-09-01';

--Write a SQL query to retrieve events with available tickets that also have "Concert" in their name.
select event_name,available_seats, event_name from Event where available_seats > 0 and event_name like '%Concert%';
...
select event_name, total_seats from Event where total_seats > 15000;
...
update Booking set num_tickets = 0 where booking_id
...
GROUP BY c.customer_id, customer_name, b.event_id;
...
update Booking set num_tickets = 150 where booking_id = 1;
select * from Booking;
SELECT 
    event_id,
    event_name
...
FROM Booking
WHERE num_tickets NOT IN (
    select num_tickets 
    FROM Booking where num_tickets != 0
);
select * from Booking;
...
SELECT 
    FORMAT(booking_date, 'MM') AS month,
    c.customer_id,
    c.customer_name,
    COUNT(*) AS tickets_booked
FROM Booking b
JOIN Customer c ON b.customer_id = c.customer_id
GROUP BY FORMAT(booking_date, 'MM'), c.customer_id, c.customer_name;
